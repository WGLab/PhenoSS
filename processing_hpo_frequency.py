...
import numpy as np
import json
with open('./doc/database/omim_conversion.json', 'r') as f:
    omim_dict = json.load(f)
with open('./doc/database/orphanet_conversion.json', 'r') as f:
...
gene2phen = pd.read_csv('./doc/database/genes_to_phenotype.txt', sep = '\t')
...
gene2phen.to_csv('./doc/database/hpo_frequency.csv', sep = '\t', index=False)