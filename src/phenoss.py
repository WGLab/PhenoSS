...
def build_output_df(rank_patients, hpo_db: pd.DataFrame, gene_conversion: bool):
    ...
    df_out["num_candidates"] = df_out.groupby("patient_id")["patient_id"].transform("size")
    return df_out
...
def parse_args():
    ...
    p.add_argument("--hp_db_sqlite", default="./databases/hp.db", type=str, help="Path to hp.db for ssmpy.")
    p.add_argument("--hpo_db_path", default="./databases/hpo_frequency.csv", type=str, help="Path to hpo_frequency.csv (tab-separated).")
    p.add_argument("--gene_conversion", action="store_true", help="Convert Mondo IDs to Genes")
    p.add_argument("--hpo_removal", action="store_true", help="Enable to remove less-informative hpo terms such as HP:0000118")
...
def main():
    ...
    # -------------------------------------------------
    if args.hpo_removal:
        print("Removing less informative HPO")
